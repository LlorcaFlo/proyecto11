    ...
    if (! isset ($_POST['nickname']) OR empty($_POST['nickname'])) {
                $val->errores['nickname'] = 'Hay que introducir un nick.<br>';

        //Si $_POST['nombre'] no es validado por el objeto de la clase Validacion...
        } elseif (! $val->va_nick($_POST['nickname'])) {
               $val->errores['nickname'] = 'El nick debe ser al menos de 6 caracteres.<br>';
        } elseif (! $val->nickUnico($_POST['nickname'])) {
               $val->errores['nickname'] = 'El nick ya está en uso, introduzca otro.<br>';
        }
